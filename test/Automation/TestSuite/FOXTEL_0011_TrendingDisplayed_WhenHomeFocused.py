...
def test_body(test_stb):
    '''
    Test point:
        When 'Home' focused, 'Trending' is displayed under 'Home'
    Test steps:
        1. Launch Home screen
        2. Focus on 'Home'
    Expected:
        'Trending' displayed under 'Home'
	'''
    test_stb.launch_HomeScreen()

    if test_stb.homefocused_get_second_category() != "Trending":
        test_stb.reportFail("Trending not displayed!")

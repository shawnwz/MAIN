...
$util.ControlEvents = (function () {
    ...
    function _add(control, eventName, callback, ctx) {
        ...
        if (typeof eventName === "string") { // register one
            events = [ eventName ];
        } else if (typeof eventName === "object") { // register many
            events = eventName;
        }

        for (i in events) { // register many
            if (events.hasOwnProperty(i)) {
                if (!_eventMap[control].callbacks[events[i]]) {
                    _eventMap[control].callbacks[events[i]] = [];
                }

                _eventMap[control].callbacks[events[i]].push({
                    callback: callback,
                    context : ctx
                });
            }

...
$service.MDS.Episode = (function Episode () {
   ...
   * @method _fetchBTV
   */
  function _fetchBTV (editorial, token) {
    var promise,
      config = {},
    ...
    if ($service.MDS.useDummyData) {
      ...
      promise = $util.fetchToken(url, $service.MDS.config.timeout, token);
      ...
      promise = $service.MDS.Base.fetch(config, token);
    }
   ...
   * @method _fetchVOD
   */
  function _fetchVOD (editorial, token) {
    var promise,
      config = {},
   ...
   */
  function fetchEpisodes (editorial, token) {

    var promises = [];

    promises.push(
      _fetchBTV(editorial, token)
          .then(_mapData,
                function (data) {

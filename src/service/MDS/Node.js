$service.MDS.Node = (function Node () {
    ...
    function _fetchDummies (token) {
        ...
        console.log("Use dummy nodes", url);
    ...
    function _appendCustom (data) {
        ...
        return $util.fetch(url, $service.MDS.config.timeout) // no token!
...
//                  }
                    ...
                    for (i = 0, len = nthNode.children.length; i < len; i++) {
                        nthId = nthNode.children[i];
                        nthChildNode = nodes[nthId];
                        ...
                        if (nthChildNode && nthChildNode.nodeLayoutInfo) {
                            ...
                            if (nthChildNode.parent === nthNodeId) {  // direct child
                                ...
                                if (nthChildNode.isCollection === true) { // collection node: push it to the collections
                                    ...
                                    nthNode._subNodes[nthId] = nthChildNode;
                                    nthNode._subNodesNb++;
                                }
                            }
                        }
            ...
            if (_rootNode && _rootNode._subNodes && _rootNode._subNodesNb) {
                ...
                for (nthNodeId in _rootNode._subNodes) {
                    ...
                    name = nthNode.displayName.toLowerCase();
                    _screenNodes[name] = nthNode;

                    // should we have jumpoffs for this node? We do if there is more than one level of subnodes
                    nthNode._hasJumpOffs = false;
                    for (nthId in nthNode._subNodes) {
                        nthChildNode = nthNode._subNodes[nthId];
                        if (nthChildNode._subNodes && nthChildNode._subNodesNb > 0) {
                            nthNode._hasJumpOffs = true; // found one of the subNodes which has subNodes: yes to _hasJumpOffs
                            break;
                        }
                    }
                }
    ...
    function fetchNodes (token) {
        ...
        return _fetchCached()
          .then(function (data) {
                ...
                function (/* data */) { // failed to get cached: fetch new
                  return _fetchNew(token);
                });
    }
    ...
    function _updateAssetsCache (node) {
        ...
        return new Promise(function (resolve, reject) {
            if (node && node._assets && node._assets.length > 0) {
                ...
                _assetsCache.forEach(function (mthNode) {
                    if (mthNode.id === node.id) {
                        newSize -= mthNode._assets.length;
                        console.log("assetsCache delete the duplicate node: id is: " + mthNode.id);
                        _assetsCache.splice(_assetsCache.indexOf(mthNode), 1);
                    }
                });
                ...
                if (node._assets.length > maxSize) {
                    ...
                    console.warn("Allocated cache size too small for node [" + node.id + "] assets [" + maxSize + "<" + node._assets.length + "]! increase cacheSize!");

                    _assetsCache.forEach(function (mthNode) {
                        mthNode._assets = [];
                        mthNode._assetsCacheTime = 0;
                    });
                    _assetsCache = [];
     ...
     */
    function fetchAssets(node, token) {
        ...
        return _fetchCachedAssets(node)
          .then(function (assets) {
                ...
                function () {
                    ...
                    return service.Asset.fetch(node, token)
                        .then(function (/* data */) {
                             /* iQ3 behaviour
                                if (node._collections && node._collections.length > 0) {
                                    ...
                                    node._assets = node._collections.concat(data);
                                } */
                                return node._assets;
                            },
                            function (data) {

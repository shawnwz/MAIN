...
app.gui.controls.HtmlTable.prototype._fetch = function _fetch () {
    ...
    if (this._config) {
        $util.fetch(this._config, 3000) // no token
            .then(function (data) {
...
app.gui.controls.HtmlTable.prototype._onKeyDown = function _onKeyDown (e) {
    ...
    // check quicks keys
    var selectedItem = this.querySelector('[data-key="' + e.key + '"]'),
        col = selectedItem ? selectedItem.itemColIndex : -1,
        row = selectedItem ? selectedItem.itemRowIndex : -1;
    if (col > -1 && row > -1) {
        this._select(row, col); // move highlight
        this.fireControlEvent("enter", this); // enter

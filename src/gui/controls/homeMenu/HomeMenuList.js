...
app.gui.controls.HomeMenuList.prototype._change = function _change() {
    ...
    if (node && node._parentNode && node._parentNode._parentNode) { // have parent and grandparent: not root
        $util.ControlEvents.fire("app-home:homeNavBreadcrumb", "populate", node._parentNode.displayName);
        $util.ControlEvents.fire("app-home:homeNavBreadcrumb", "show");
    } else {
        $util.ControlEvents.fire("app-home:homeNavBreadcrumb", "hide");
    }
